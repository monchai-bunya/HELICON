...
def rs_info_update():
	...
	while True:
		...
		fd = open('request_time.txt','r')
		tmp = os.popen("ps -A").read()
		if process_name not in tmp[:]:
		   request_time = '0'
		else:
		   request_time = fd.read()
		#get all values in a variable
		record = '%s %s %s %s %s %s' % (norm_load, mem_utilization, bandwidth, net_utilization, device_type, request_time.strip())
		print(record)
		time.sleep(5)
global record
