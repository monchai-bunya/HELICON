...
dataset_full_t = [ttotal1,ttotal2,ttotal3,ttotal4]
dataset_full_l = [load1,load2,load3,load4]
dataset_full_tt = [i[0] for i in sorted(enumerate(dataset_full_t), key=lambda x:x[1])]
dataset_full_lt = [i[0] for i in sorted(enumerate(dataset_full_l), key=lambda x:x[1])]
#dataset_full_ts = sorted(range(len(dataset_full_t)),key=dataset_full_t.__getitem__)
#dataset_full_ls = sorted(range(len(dataset_full_l)),key=dataset_full_l.__getitem__)
for n in range(0, len(dataset_full_tt)):
  dataset_full_ts.append(dataset_full_tt.index(n))

for n in range(0, len(dataset_full_lt)):
  dataset_full_ls.append(dataset_full_lt.index(n))

dataset_full_tl = [(x*weight_time) + (y*weight_load) for x, y in zip(dataset_full_ts,dataset_full_ls)]
#dataset_full_tl = [x + y for x, y in zip(dataset_full_ts,dataset_full_ls)]


#disconnect2
#if 1 in dlist:
#  dlist.remove(1)
#  dataset_full_tl[1] = 10

#disconnect3
#if 2 in dlist:
#  dlist.remove(2)
#  dataset_full_tl[2] = 10

#disconnect4
#if 3 in dlist:
#  dlist.remove(3)
#  dataset_full_tl[3] = 10
