...
def rs_info_generate(rs):
	rs_multicast_group = ('224.3.29.74', 30002)
...
def ras_list():
	...
	addresses = config_dic['PING_ADDRESS_UE']
	tcap = open('outtempuelist.csv','w')
	cap = open('time_request.txt','w')
	tcap2 = open('latency_list.csv','a')
	cap2 = open('latency_request.txt','w')

	outfileml = config_dic['UE_FILENAME_ML']	
	mvout = 'mv outtempuelist.csv ' + outfileml
	...
	for i in ad:
		...
		if rs_info.has_key(i):
			...
			#print '%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t\tName:%s' % ('0.5', '50', latency, pkloss, jitter, '27', '50', vnfload, '31', i)
			save_info = '%s,%s,%s,%s,%s,%s,%s,%s,%s,%s\n' % ('0.5', '50', latency, pkloss, jitter, '27', '50', vnfload, i, '31')
			tm_transcode = '0'
		tcap.write(save_info)
		cap.write(tm_transcode)
		tcap2.write(str(latency))
		tcap2.write('\n')
		cap2.write(str(latency))
